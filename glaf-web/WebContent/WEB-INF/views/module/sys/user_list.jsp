...
<%
	...
	SysDepartment department = (SysDepartment) request
...
<html>
	<head>
		...
		<title></title>
		<link href="<%=context%>/css/site.css" type="text/css"
			rel="stylesheet">
		<script language="javascript" src='<%=context%>/js/verify.js'>
</script>
		<script language="javascript" src='<%=context%>/js/main.js'>
</script>
...
function add() {
	var supplycode = '<%=request.getParameter("supplycode")%>';
	if ("SUPPLY" == supplycode) {
		var url = "<%=context%>"+"/addUser.do?parent=<%=department.getId()%>";
	} else {
...
function addHr(){
	 var url="user.do?method=getHRUser&parent=<%=department.getId()%>";
	  var width=450;
	  var height=450;
	  var scroll="no";
	  openWindow(url, width, height, scroll);
	...
	<body>
		...
		<html:form action="/sys/user.do?method=batchDelete" method="post"
			target="_self">
			<input name="page_no" type="hidden"
				value="<%=pager.getCurrentPageNo()%>">
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="0"
							...
							while (iter.hasNext()) {
								...
								if (userRoles != null) {
									...
									while (roleIter.hasNext()) {
										SysUserRole role = (SysUserRole) roleIter
												.next();
										if (role.getDeptRole() != null
												&& role.getDeptRole().getDept() != null
												&& role.getDeptRole().getRole() != null) {
											roleName += role.getDeptRole().getDept()
													.getName()
													+ role.getDeptRole().getRole()
															.getName();
										}
				...
				<tr <%=i % 2 == 0 ? "" : "class='list-back'"%>>
					...
					<td class="td-no"><%=((pager.getCurrentPageNo() - 1) * pageSize
								+ i + 1)%></td>
					<td class="td-text"><%=bean.getAccount()%>&nbsp;
					</td>
					<td class="td-text"><%=bean.getName()%>&nbsp;
					</td>
					<td class="td-c"><%=bean.getDepartment().getName()%>&nbsp;
					</td>
					<td class="td-text" title="<%=roleName%>"><%=roleName%>
					</td>
					<td class="td-no"><%=bean.getBlocked() == 1 ? "否" : "是"%>&nbsp;
					</td>
					<td class="td-time"><%=new java.text.SimpleDateFormat("yyyy-MM-dd")
								.format(bean.getCreateTime())%>&nbsp;
					</td>
					<td align="center" class="list"><%=bean.getLastLoginTime()==null?" ":new java.text.SimpleDateFormat("yyyy-MM-dd mm:ss").format(bean.getLastLoginTime())%>&nbsp;</td>
				</tr>
				<%
					i++;
							}
			...
			<table width="100%" border="0" align="center" cellpadding="0"
				...
				<tr>
					<td width="60%">
						<input name="btn_add" type="button" value="增加" class="button"
							onClick="javascript:add();">
						<input name="btn_del" type="button" value="删除" class="button"
							onClick="javascript:del();" disabled>
						<input name="btn_modify" type="button" value="修改" class="button"
							onClick="javascript:modify(this.form);" disabled>
						<input name="btn_reset_pwd" type="button" value="重置密码"
					...
					<td width="50%">
						...
						<jsp:include page="/WEB-INF/views/inc/show_page.jsp" flush="true">
							...
							<jsp:param name="page_count"
								value="<%=pager.getTotalPageCount()%>" />
							...
							<jsp:param name="params"
								value="<%=java.net.URLEncoder.encode(params)%>" />
						</jsp:include>
					</td>
				</tr>
			</table>
