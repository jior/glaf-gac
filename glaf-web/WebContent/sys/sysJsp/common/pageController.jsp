...
<%@ taglib prefix="bean" uri="/sys/sysTld/struts-bean.tld" %>
	...
	<tr>
		...
		<td>
			<bean:message key='pageCtrl.Count'/><html:text style="width:30px " property="pageCounts" maxlength="3" onblur="textBlur(this)"/>
		</td>
		<td>	
...
<script type="text/javascript" language="Javascript"> 
	...
	function textBlur(myText) {
		myText.value = myText.value.replace('.','');
		var re = /^\d+(?=\.{0,1}\d+$|$)/;
		 if(!re.test(myText.value)){
			 myText.value='';
		 } 
	}
