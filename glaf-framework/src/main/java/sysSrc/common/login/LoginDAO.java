				...
				if (tUser.getFPassword().equals(form.getPassword())) {
					...
					if ("1".equals(tUser.getFStatus())) {
						...
						logger.info("tUser.getId():" + tUser.getFUserid());
						 ...
						 JSONArray array = bean.getUserMenu(3, tUser.getFUserid());
						 logger.info("JSON MENU:" + array.toString());
						
						HashMap<String, List> rolePrivilageMap = getRolePrivilageMap(form.getUserId());
						...
						sysBaseCom.setAllPrivilages(getAllPrivilageIds(sysBaseCom.getAllPrivilege()));
						// 获取当前登录者拥有的所有权限，并放入session中
						sysBaseCom.setUserPrivilege(getAllPrivilageList(tUser.getFUserid()));
		...
		for (TUsersRole role : usersRoleTypeList) {
			...
			List<TRolePrivilege> rolePrivilegeTypeList = (List<TRolePrivilege>) this.dbAccess
					.find(
							...
							" WHERE A.id.TRole.FRoleid=:roleid order by A.id.TPrivilege.FPrivilegeid,A.id.TPrivilege.FPrivilegefather ",
							paramsP);
			...
			for (TRolePrivilege privilege : rolePrivilegeTypeList) {
				...
				if (rolePrivilageMap.containsKey(s)) {
					...
					l.add(privilege.getId().getTPrivilege().getFPrivilegeid()
							.toUpperCase()
							+ ","
							+ privilege.getId().getTPrivilege().getFPrivilegename()
							+ ","
							+ privilege.getId().getTPrivilege().getFUrl()
							+ ","
							+ privilege.getId().getTPrivilege().getFPrivilegefather()
							);
	...
	private List<TPrivilege> getAllPrivilageList(String userid) {
		...
		if (null != userid && !"".equals(userid)) {
			...
			allPrivilageTypeList = (List<TPrivilege>) this.dbAccess.find(
					...
					" WHERE 1=1 order by A.FPrivilegeid,A.FPrivilegefather ", params);
		}
