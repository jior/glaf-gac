...
public class MailNotify {
	...
	public MailNotify() {
		try {
			...
			String repeatmai = mailserver.getMailAddr();
	...
	public void sendmail(List recipients, String subject, String content,
		...
		// ISC)TXT 2013-1-22 邮件发送失败 start
		// String fromString = mailserver.getUsername() + "@"
		// + mailserver.getServerip();
		String fromString = mailserver.getMailAddr();
		// ISC)TXT 2013-1-22 邮件发送失败 end
		...
		// 认证
		Email_Autherticatorbean auth = new Email_Autherticatorbean(
		...
		try {
			...
			// 设置回复邮件地址
			if ("".equals(getReplyTo())) {
				msg.setReplyTo(InternetAddress.parse(mailserver.getRepeatmail()));
			} else {
	...
	private InternetAddress[] getRecipientsAddress(List recipients)
		...
		if (recipients != null && recipients.size() > 0) {
			...
			for (int i = 0; i < recipients.size(); i++) {
				recipientsaddress[i] = new InternetAddress(
						String.valueOf(recipients.get(i)));
			}
		}
	...
	private MimeMultipart getMailpart(String mailtext, List files)
		...
		if (files != null && files.size() > 0) {
			...
			for (int i = 0; i < files.size(); i++) {
				...
				try {
					mbp2.setFileName(javax.mail.internet.MimeUtility
							.encodeText(fds.getName()));
				} catch (UnsupportedEncodingException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					throw new MessagingException("设置附件fileName失败！");
				}
